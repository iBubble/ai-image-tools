...
def parse_markdown(md_path):
    ...
    # Parse images WITH their section titles
    # Strategy: walk through lines, track current ## heading, associate images found under it
    lines = content.split('\n')
    current_heading = ""
    images = []  # list of {path, label}
    seen_paths = set()
    ...
    for line in lines:
        ...
        # Find image paths in this line
        img_matches = re.findall(r'(/Users/gemini/Projects/Own/Antigravity/AntigravityFixed/.secret/punishments/\d{8}/[\w.-]+\.(?:jpg|png|jpeg))', line)
        for path in img_matches:
            if os.path.exists(path) and path not in seen_paths:
                ...
                label = current_heading if current_heading else os.path.basename(path)
                images.append({"path": rel, "label": label})
                seen_paths.add(path)
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .m-view {{ flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; overflow: hidden; }}
        .m-view img {{ max-width: 100%; max-height: calc(100% - 30px); object-fit: contain; }}
        .img-label {{ color: var(--gold); font-size: 0.9rem; margin-top: 8px; text-align: center; min-height: 20px; }}
        .m-strip {{ height: 110px; display: flex; gap: 10px; padding: 10px; background: rgba(0,0,0,0.5); overflow-x: auto; border-top: 1px solid var(--border); }}
        .m-thumb {{ height: 100%; aspect-ratio: 1; background-size: cover; background-position: center; border: 2px solid transparent; cursor: pointer; opacity: 0.5; position: relative; transition: 0.2s; border-radius: 4px; }}
        .m-thumb:hover {{ opacity: 0.8; }}
        .m-thumb.active {{ opacity: 1; border-color: var(--gold); }}
        .m-thumb .tip {{ position: absolute; bottom: -28px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.9); color: var(--gold); font-size: 0.7rem; padding: 3px 8px; border-radius: 4px; white-space: nowrap; display: none; z-index: 10; pointer-events: none; }}
        .m-thumb:hover .tip {{ display: block; }}
...
<body>
    ...
    <div id="modal"><div class="m-body"><span class="m-close" onclick="closeM()">&times;</span>
        <div class="m-gal"><div class="m-view"><img id="v-img"><div class="img-label" id="v-label"></div></div><div class="m-strip" id="v-strip"></div></div>
    ...
    <script>
        ...
        function openM(p) {{
            ...
            p.images.forEach((imgObj, i) => {{
                ...
                t.title = imgObj.label;
                const tip = document.createElement('div'); tip.className = 'tip'; tip.innerText = imgObj.label;
                t.appendChild(tip);
                t.onclick = () => {{
    ...
    print("V6 Generated - with image section tooltips.")

if __name__ == "__main__": generate_log_v6()
